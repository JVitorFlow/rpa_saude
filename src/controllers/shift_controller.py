...
from src.controllers.anatomopatologico_controller import \
    extrair_dados_anatomopatologico
from src.controllers.api_handler import (atualizar_item_erro_shift,
                                         atualizar_item_fim,
                                         atualizar_item_inicio,
                                         atualizar_tarefa_inicio,
                                         enviar_dados_api)
from src.controllers.buscar_numero_recipiente import \
    buscar_prefixo_numero_recipiente
from src.controllers.endereco_controller import extrair_dados_endereco
from src.controllers.navigation_handler import (acessar_informacoes_paciente,
                                                buscar_os_no_sistema,
                                                esperar_tela_manutencao,
                                                fechar_janela_exame,
                                                fechar_janela_manutencao)
from src.controllers.paciente_controller import (extrair_dados_paciente,
...
class ShiftController:
    ...
    def __init__(
        self, url, usuario, senha, screenshot_path, api_client, robot_id
    ):
    ...
    def processar_dados(self, tasks):
        ...
        for task in tasks:
            ...
            if not os_numero or not nome_pessoa:
                logger.warning(
                    f'Tarefa {task_id} está incompleta: OS ou nome ausente.'
                )
                continue
            ...
            dados_extraidos = self._extrair_dados_do_shift(
                os_numero, nome_pessoa
            )
            if not dados_extraidos:
                continue

            enviar_dados_api(
                self.api_client, task_id, item_id, dados_extraidos
            )
            atualizar_item_fim(
                self.api_client,
                item_id,
