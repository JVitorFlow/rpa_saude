...
def enviar_dados_api(api_client, task_id, item_id, dados):
    ...
    try:
        ...
        if response:
            ...
            logger.error(
                f'Falha ao enviar dados para a API para tarefa {task_id}.'
            )
...
def atualizar_item_fim(api_client, item_id, status, shift_result, stage=None):
    ...
    logger.info(
        f"Atualizando item {item_id} para '{status}' com stage '{stage}'."
    )
...
def atualizar_item_sismama(api_client, item_id):
    ...
    logger.info(
        f"Atualizando item {item_id} para 'COMPLETED' no estágio 'SISMAMA'."
    )
    response = api_client.update_item(item_id, **dados)
...
def atualizar_item_erro_sismama(api_client, item_id, mensagem_erro):
    ...
    logger.info(
        f"Atualizando item {item_id} para 'ERROR' no estágio 'SISMAMA'."
    )
    response = api_client.update_item(item_id, **dados)
...
def atualizar_item_erro_shift(api_client, item_id, mensagem_erro):
    ...
    logger.info(
        f"Atualizando item {item_id} para 'ERROR' no estágio 'SHIFT'."
    )
    response = api_client.update_item(item_id, **dados)
