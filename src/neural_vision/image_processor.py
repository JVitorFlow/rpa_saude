...
class AutomacaoImageProcess:
    ...
    def processar_item(self, item):
        ...
        if not self._atualizar_status_item(item_id, "STARTED", "IMAGE_PROCESS"):
            return
        ...
        try:
            ...
            if result_data:
                logger.info(f"Análise concluída para OS {os_number}: {result_data}")
    ...
    def _atualizar_status_item(
        ...
        if response:
            logger.info(f"Item {item_id} atualizado para {status} na etapa {stage}.")
            return True
        else:
    ...
    def processar_pendentes(self):
        ...
        """
        logger.info("Verificando itens pendentes para processamento de imagens...")

        items_pendentes = self.api_client.get_pending_items(stage="IMAGE_PROCESS")
        ...
        for task in items_pendentes:
            ...
            logger.info(f"Processando tarefa ID: {task_id} com {len(items)} itens.")
